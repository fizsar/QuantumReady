"""Cliente, Servidor y Atacante del intercambio híbrido ML-KEM-768 + X25519.

Los papeles siguen a X25519MLKEM768 (TLS 1.3) y mlkem768x25519 (OpenSSH):

- El Cliente genera los pares ML-KEM-768 y X25519 y envía ambas públicas.
- El Servidor encapsula contra la pública ML-KEM del Cliente, hace X25519 y
  devuelve el ciphertext y su pública X25519.
- El Cliente decapsula y hace X25519.
...
class RespuestaServidor:
    """Servidor → Cliente (key_share del ServerHello)."""
...
class Cliente(Actor):
    ...
    def claves_publicas(self) -> ClavesPublicasCliente:
        mensaje = ClavesPublicasCliente(
    ...
    def recibir(self, respuesta: RespuestaServidor) -> None:
        """Decapsula el ciphertext y hace X25519 con la pública del Servidor."""
        self.secreto_mlkem = self._anotar(
            "Secreto ML-KEM (decapsulado)",
            self._mlkem.decapsulate(respuesta.mlkem768_ciphertext))
        publica_servidor = x25519.X25519PublicKey.from_public_bytes(respuesta.x25519)
...
class Servidor(Actor):
    ...
    def responder(self, claves: ClavesPublicasCliente) -> RespuestaServidor:
        """Encapsula contra la pública ML-KEM y hace X25519 con la del Cliente."""
        ...
        publica_cliente = x25519.X25519PublicKey.from_public_bytes(claves.x25519)
        self.secreto_x25519 = self._anotar(
...
class Atacante(Actor):
    ...
    def __init__(self, registro: list[Evento] | None = None):
        super().__init__(registro)
        self.cliente: ClavesPublicasCliente | None = None
        self.servidor: RespuestaServidor | None = None

    def observar(self, cliente: ClavesPublicasCliente,
                 servidor: RespuestaServidor) -> None:
        self.cliente, self.servidor = cliente, servidor
    ...
    def intentar(self) -> list[Intento]:
        ...
        secreto_kem = propia_kem.decapsulate(self.servidor.mlkem768_ciphertext)
        secreto_x = propia_x.exchange(
            x25519.X25519PublicKey.from_public_bytes(self.cliente.x25519))
        ...
        con_datos_publicos = derivar_clave_final(
            secreto_mlkem=self.servidor.mlkem768_ciphertext[:32],
            secreto_x25519=self.cliente.x25519)

        # 3. Encapsular de nuevo contra la pública ML-KEM del Cliente: solo
        #    produce un secreto NUEVO, no el que encapsuló el Servidor.
        nuevo_secreto, _ = mlkem.MLKEM768PublicKey.from_public_bytes(
            self.cliente.mlkem768).encapsulate()
        ...
        return [
            ...
            Intento("re-encapsular contra la pública ML-KEM del Cliente",
                    con_reencapsulado),
        ]
